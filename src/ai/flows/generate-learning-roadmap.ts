...
const WeeklyPlanSchema = z.object({
    week: z.number().describe("The week number (e.g., 1, 2, 3)."),
    summary: z.string().describe("A brief summary of the week's focus."),
    dailySchedule: z.array(DailyPlanSchema).describe("The schedule for each day of the week.")
});

const MonthlyPlanSchema = z.object({
    month: z.string().describe("The name of the month (e.g., 'January')."),
    weeklyPlans: z.array(WeeklyPlanSchema).describe("An array of weekly plans for the month.")
});
...
const GenerateLearningRoadmapOutputSchema = z.object({
  ...
  monthlyPlan: MonthlyPlanSchema.describe("The structured learning plan for one month."),
});
...
export async function generateLearningRoadmap(input: GenerateLearningRoadmapInput): Promise<GenerateLearningRoadmapOutput> {
  ...
  return {
    ...
    monthlyPlan: {
        ...
        weeklyPlans: [{
            week: 1,
            summary: "Focus on core fundamentals and address one major weak area.",
            dailySchedule: [
            {
                day: 'Monday',
                sessions: [
